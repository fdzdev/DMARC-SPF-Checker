...
def get_custom_resolver():
    ...
    resolver.nameservers = ["8.8.8.8", "1.1.1.1"]  # Google's and Cloudflare's DNS
    resolver.timeout = 5  # Timeout for individual DNS queries
    resolver.lifetime = 10  # Total time allowed for each query
    return resolver


# Check DMARC policy using the custom resolver
def check_dmarc_policy(domain):
    try:
        resolver = get_custom_resolver()
...
# Check SPF record using the custom resolver
...
# Log the results of the domain scan
